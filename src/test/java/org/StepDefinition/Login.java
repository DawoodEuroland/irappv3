...
String first;
    ...
    public void to_check_whether_user_can_be_able_to_delete_the_account_using_delete_button() {
        ...
        swipeUsingText("Delete Account");
Click(login.getDeleteAccountButton(),10);
Click(login.getDeleteButton(),10);
        System.out.println("User has successfully deleted his google account");
    }
...
first = fname;
        ...
        public void to_click_the_verify_captcha_screen() throws InterruptedException {
            ...
            sleep(3000);
            try{
                if (login.getCheckboxCaptcha().isDisplayed()){
...
char firstIcon = first.toUpperCase().charAt(0);
        Click(login.getFirstNameIcon(firstIcon),10);
        swipeUsingText("Sign out");
        Click(login.getSignOutButton(),10);
        System.out.println("User is successfully able to signout from his account");
    }
